...
int assemble_bfchan_jump_backward_operator (bfchan_operator *operator, bfchan *bfchanp, nanafy *nana){
	size_t offsetbegin = tell_nanafy(NANAFY_TEXT_SECTION, nana);
	if (write_nanafy(BFCHAN_JUMP_BACKWARD_OPERATOR_ASSEMBLY, sizeof(BFCHAN_JUMP_BACKWARD_OPERATOR_ASSEMBLY), NANAFY_TEXT_SECTION, nana) != 0){
		return 1;
	}
