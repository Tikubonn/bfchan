#include <bfchan.h>
#include <nanafy.h>
#include "assemble-bfchan-jump-backward-operator.p"

nanafy_status assemble_bfchan_jump_backward_operator_manually (bfchan_operator *operator, bfchan *bfchanp, nanafy *nana){
	size_t offsetbegin = tell_nanafy(NANAFY_TEXT_SECTION, nana);
	nanafy_status status;
	status = write_nanafy_manually(BFCHAN_JUMP_BACKWARD_OPERATOR_ASSEMBLY, sizeof(BFCHAN_JUMP_BACKWARD_OPERATOR_ASSEMBLY), NANAFY_TEXT_SECTION, nana);
